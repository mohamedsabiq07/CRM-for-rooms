        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
          ...
          <div>
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full text-sm pl-9 pr-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-400 font-mono"
              />
            </div>
          </div>
            ...
            <div className="flex items-center justify-between gap-3 text-xs">
              ...
              {!doorKeyReturned && (
                <div className="flex items-center gap-1.5">
                  <span className="text-rose-700 text-[11px] font-bold">Lost Fee: AED</span>
                  <input
                    type="number"
                    value={doorKeyFee}
                    onChange={(e) => setDoorKeyFee(e.target.value)}
                    className="w-16 px-2 py-1 border border-rose-300 rounded-lg text-xs font-bold text-rose-800 bg-white"
                  />
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <input
                ...
                placeholder="0"
                className="w-full text-xs px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-400 font-semibold text-slate-800"
              />
            </div>
          ...
          <div className={`p-4 rounded-2xl border-2 transition ${
            ...
            {settlement.totalDeductions > 0 && (
              <div className="text-[11px] text-slate-600 space-y-0.5 pb-2 border-b border-slate-200/80 mb-2">
                {settlement.unpaidRentDeduction > 0 && (
                  <div className="flex justify-between">
                    <span>Unpaid Rent:</span>
                    <span className="text-rose-600">-AED {settlement.unpaidRentDeduction}</span>
                  </div>
                )}
                {settlement.totalKeyCharges > 0 && (
                  <div className="flex justify-between">
                    <span>Lost Key Charges:</span>
                    <span className="text-rose-600">-AED {settlement.totalKeyCharges}</span>
                  </div>
                )}
                {settlement.damageCharges > 0 && (
                  <div className="flex justify-between">
                    <span>Damages/Cleaning:</span>
                    <span className="text-rose-600">-AED {settlement.damageCharges}</span>
                  </div>
                )}
              </div>
            )}
            ...
            <div className="flex items-baseline justify-between pt-1">
              ...
              <span className={`text-2xl font-black ${
                settlement.giveBackAmount > 0 ? 'text-emerald-700' : settlement.tenantOwesExtra > 0 ? 'text-rose-700' : 'text-slate-800'
              }`}>
          ...
          <div>
            ...
            <input
              ...
              className="w-full text-xs px-3 py-2 border border-slate-300 rounded-xl focus:outline-none focus:ring-2 focus:ring-rose-400"
            />
          </div>
          ...
          <div className="pt-3 border-t border-slate-200 flex items-center justify-end gap-2.5">
            ...
            <button
              type="submit"
              className="px-5 py-2 text-xs font-bold bg-rose-600 hover:bg-rose-500 text-white rounded-xl shadow-lg shadow-rose-600/20 transition flex items-center gap-1.5"
            >
