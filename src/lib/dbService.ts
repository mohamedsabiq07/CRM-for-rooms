...
export async function fetchTenantsFromDb(): Promise<Tenant[]> {
  try {
    ...
    return data.map(d => ({
      ...
      stayMonth: d.stay_month || 'Sep-2026',
      monthStatusHistory: d.month_status_history || {},
      remarks: d.remarks || '',
...
export async function upsertTenantToDb(tenant: Tenant) {
  try {
    await supabase.from('room_crm_tenants').upsert({
      ...
      stay_month: tenant.stayMonth || 'Sep-2026',
      month_status_history: tenant.monthStatusHistory || {},
